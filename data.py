...
def statewise_analysis():
    ...
    for states in state_data:
        ...
        for i in range(len(state_names)):
            states_dict.update({state_names[i]: [
                               statewise_confirmed[i], statewise_active[i], statewise_recovered[i], statewise_deaths[i]]})
        states_sorted_list = sorted(states_dict.items(), key=lambda kv: [
    ...
    for item in states_sorted_list:
        ...
        state_report_list = [final_state_names, final_statewise_confirmed,
                             final_statewise_active, final_statewise_recovered, final_statewise_deaths]
    del state_names
...
def district_zone_analysis():
    import requests
    district_data = requests.get(
        "https://api.covid19india.org/zones.json").json()
    states_district = {}
    states_district_zone = {}
    for item in district_data['zones']:
        if item['states'] not in states_district:
            states_district[item['state']] = [item['district']]
            states_district_zone[item['state']] = {
                item['district']: [item['lastupdated'], item['zone']]}
        else:
            states_district[item['state']].append(item['district'])
            states_district_zone[item['state']].update(
                {item['disctrict']: [item['lastupdate'], item['zone']]})
