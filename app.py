...
def covid():
    ...
    return render_template("index.html", data = india_data, world = countries_data_all)
...
def countries():
    ...
    for item in countries_data['locations']:
        if item['country'] not in countries_dict:
            countries_dict.update({item['country']: {'Population': item['country_population'],
                                                     'Confirmed': item['latest']['confirmed'], 'Deaths': item['latest']['deaths']}})
        else:
...
def india():
    from data import daywise_data_india
    ...
    from data import statewise_analysis
...
def daywise_data_india():
    ...
    india_plot_daywise_data = daywise_data_india()
    day = india_plot_daywise_data[0]
    confiremd = india_plot_daywise_data[1]
    deaths = india_plot_daywise_data[2]
